# -*- coding: utf-8 -*-
"""
Created on Sun Nov 20 21:19:12 2022
...
stopwords_path = './results/hit_stopwords.txt'
...
pretrained_path = './results' #预训练词向量存放路径
...
train_iter, dev_iter,test_iter = torchtext.data.BucketIterator.splits(
        ...
        batch_sizes=(128, 128, 1), # 每个迭代器分别以多少样本为一个batch
...
steps_eval = 10000  # 每100步测试一下验证集的准确率
early_stopping = 100000  # 若发现当前验证集的准确率在1000步训练之后不再提高 一直小于best_acc,则提前停止训练
...
textcnn_model = TextCNN(class_num=class_num,
        ...
        vectors=vectors,
        dropout=dropout)
#textcnn_model.load_state_dict(torch.load("./results/bestmodel_steps1600.pt"))
import os
train(train_iter,dev_iter,textcnn_model)
def predict(text):
    line = "0" + "\t" + text.strip('\n')[0:40]
    with open('./results/text.tsv','w+',encoding="utf8")as t:
        t.write(line +'\n')
    t.close()
    train_dataset,dev_dataset,test_dataset = torchtext.data.TabularDataset.splits(
          path='./results',  #文件存放路径
    ...
    for batch in test_iter:
        ...
        prediction = labels[list(logits[0]).index(max(logits[0]))]
    # --------------------------------------------------------------------------
        return prediction
